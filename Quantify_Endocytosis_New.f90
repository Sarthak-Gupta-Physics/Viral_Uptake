...
! This code quantifies the trajectory data that is generated by Trig_March.f90 !
! This analyze the final shape of the cell surface, while it is wrapped around the 
! virus. It calculates the shape index, cell surface area used in uptake,  !
! the virus wrapping or how much virus surface is covered with the membrane !
! and velocity of viral uptake. !
...
implicit none
	...
	Real*8  :: probe_dis
	Real*8 :: High_Diff,High_Diff_2,Hr
...
integer :: i
  ...
  PIE=4.D0*DATAN(1.D0)    ! Value for Pie
  Sigma_VIM=1.d0 
...
Real*8 :: Whole_Surface_Area
	Real*8 :: Vx_p2p3,Vy_p2p3,Vz_p2p3,mag_p2p3,per,s,mag_p1p2,mag_p1p3		
Real*8,allocatable,dimension(:,:) :: Boundary,Boundary_Temp
Integer,allocatable,dimension(:) :: Bulge_Boundary	
Real*8 :: Perimeter,Shape_Index
...
! Band Disect	
Max_Rad=19.0!14.d0!3.0*(5.0)!sqrt(360.d0*360.d0 + 180.d0*180.d0)
...
Allocate(Boundary(N_Section,TM))
Allocate(Boundary_Temp(N_Section,TM))	
Allocate(Bulge_Boundary(N_Section))
Do I=1,N_Section
	Do J=1,TM
		Boundary(I,J)=0.d0
		Boundary_Temp(I,J)=0.d0
	End Do
End DO
...
Do I=1,TM	
	...
	Do K=1,N_Section
		IF((Surface_Angle.GT.((K-1)*Increment)).AND.(Surface_Angle.LE.(K*Increment))) then
			Section_ID(I)=K
			Boundary_Temp(K,I)=Dis	
		End IF
	End Do
End Do


H_Cut_Off=0.d0!RZ(Comp_Spike)-RZ(CL+1)!RZ(CL+1)-RZ(POS_ACE2)!Top_Z_Virus+1.d0
Point_Count=0
	Do K=1,N_Section
		Flag_Exit=0
		Do J=1,N_Band
			...
			Do While((Flag_Exit==0).AND.(I.LE.TM))
				IF((Section_ID(I)==K).AND.(Band_ID(I)==J)) Then
					IF( (H(I).GE. H_Cut_Off) ) then
						Point_Count=Point_Count+1
						Flag_Bowl(I)=1
						write(444,*) RX(I),RY(I),H(I)
						Boundary(K,I)=Boundary_Temp(K,I)
					Else
...
Do I=1,TM	
	IF((Flag_Bowl(I)==1).and.(Flag_6(I)==0)) then
		Do J=1,6
			...
			mag_p1p2=sqrt(Vx_p1p2*Vx_p1p2 + Vy_p1p2*Vy_p1p2 + Vz_p1p2*Vz_p1p2)
			...
			mag_p1p3=sqrt(Vx_p1p3*Vx_p1p3 + Vy_p1p3*Vy_p1p3 + Vz_p1p3*Vz_p1p3)
			
			Vx_p2p3=rx(p3)-rx(p2)
			Vy_p2p3=ry(p3)-ry(p2)
			Vz_p2p3=rz(p3)-rz(p2)
			
			mag_p2p3=sqrt(Vx_p2p3*Vx_p2p3 + Vy_p2p3*Vy_p2p3 + Vz_p2p3*Vz_p2p3)
			
			per=mag_p1p2+mag_p1p3+mag_p2p3
			s=per/2.d0
			
			Area=sqrt(s*(s-mag_p1p2)*(s-mag_p1p3)*(s-mag_p2p3))
...
!Find the boundary points and find perimeter
Open(Unit=1005,file="BB.dat")

		Bulge_Boundary=MAXLOC (Boundary, DIM=2)

		Do I=1,N_Section
			K=Bulge_Boundary(I)
			write(1005,*) RX(K),RY(K),H(K) 
		End Do		

Perimeter=0.d0
		Do I=1,N_Section-1
			XR=RX(Bulge_Boundary(I+1))-RX(Bulge_Boundary(I))
			YR=RY(Bulge_Boundary(I+1))-RY(Bulge_Boundary(I))
			ZR=RZ(Bulge_Boundary(I+1))-RZ(Bulge_Boundary(I))
			
			Perimeter=Perimeter+sqrt(XR*XR + YR*YR + ZR*ZR)
		End Do		
Shape_Index= Perimeter/sqrt(Membrane_Surface_Area)
	
write(333,*)Ben,VD,Point_Count,Point_Count/Real(TM),Membrane_Surface_Area/Whole_Surface_Area,Perimeter,Shape_Index,1.d0/Shape_Index
Return
...
Real*8,dimension(Comp_Spike) :: Heighst_Z
...
Count_Stick=0
	...
	Top_Z_Virus=maxloc(Heighst_Z,1)
!_________________________________________________________!
...
! Add all those area patches!
	...
	Theta_Max=360.d0
	Theta_Min=0.d0
	Theta_Inc=7.25*2.255!16.25!8.089!2.5


	Phi_Max=180.d0
	Phi_Min=0.d0
	Phi_Inc=7.25*2.255!16.25!8.089!2.5
...
Do I=1,Phi_Sec ! Phi loop
		...
		Phi_Up=(I)*Phi_Inc*(PIE/180.d0)
			...
			!			print*,i,j,K,Area_Mesh(I,J)!,theta_low*(180.d0/pie),theta_up*(180.d0/pie),phi_low*(180.d0/pie),phi_up*(180.d0/pie)
...
k=k+5	
	End Do
	write(120,*) Ben,VD,Total_SA/(4*pie*Raidus_Org_Avg*Raidus_Org_Avg),Count_Stick
return
...
implicit none
	...
	filename110='Mem'//trim(adjustl(Ben_ch))//'_'//trim(adjustl(Sp_no_Ch))//'_' &
	   //trim(adjustl(VD_Ch))//'_'//trim(adjustl(Ens_Ch))//'.dat'
	filename13='Wrapping'//trim(adjustl(Ben_ch))//'_'//trim(adjustl(Sp_no_Ch))//'_' &
	   //trim(adjustl(VD_Ch))//'_'//trim(adjustl(Ens_Ch))//'.dat'
	filename14='Membrane_Area'//trim(adjustl(Ben_ch))//'_'//trim(adjustl(Sp_no_Ch))//'_' &
	   //trim(adjustl(VD_Ch))//'_'//trim(adjustl(Ens_Ch))//'.dat'
	filename16='Velocity'//trim(adjustl(Ben_ch))//'_'//trim(adjustl(Sp_no_Ch))//'_' &
	   //trim(adjustl(VD_Ch))//'_'//trim(adjustl(Ens_Ch))//'.dat'
